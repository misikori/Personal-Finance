...
public class BudgetServiceClient : IBudgetService
{
    ...
    private async Task<string> GetUserWalletIdAsync(string userId, string currency)
    {
        try
        {
            ...
            {
                _logger.LogError("No wallet ID returned for user {UserId}", userId);
                throw new InvalidOperationException($"User {userId} has no wallets in Budget service");
            }
            
            _logger.LogDebug("Using wallet {WalletId} (Currency: {Currency}) for user {UserId}", 
        ...
        {
            _logger.LogError(ex, "gRPC error getting wallet for user {UserId}. Status: {Status}", 
    ...
    public async Task<bool> HasSufficientFundsAsync(string username, decimal amount, string currency)
    {
        try
        {
            ...
            _logger.LogInformation(
                "Budget check for {Username}: Balance={Balance} {Currency}, Required={Required}, HasEnough={HasEnough}",
                username, response.CurrentBalance, currency, amount, hasEnough);
